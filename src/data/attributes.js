...
export const attributes = applyFilters( 'formBlock.data.attributes', {
	...
	disabled: {
		controlType: 'toggle',
		description: __(
			'Whether the form element is disabled and will not be submitted by sending the form.',
			'form-block'
		),
	...
	isReplyTo: {
		controlType: 'toggle',
		description: __(
			'Whether to use the value of this field as reply-to in the email to allow automatically answer the user via email.',
			'form-block'
		),
		hideLink: true,
	...
	pattern: {
		controlType: 'text',
		description: __(
			'A pattern the value must match to be valid. Any valid regular expression can be used (without forward slashes in the beginning or the end).',
			'form-block'
		),
		examples: [ 'value', 'd+', '(.*){10,20}' ],
		label: _x( 'Pattern', 'HTML attribute name', 'form-block' ),
	},
	placeholder: {
		controlType: 'text',
		description: __(
			'A placeholder is displayed in the form element before any value is added to it.',
			'form-block'
		),
	...
	readOnly: {
		controlType: 'toggle',
		description: __(
			'If the form element is set to readonly, it cannot be edited but you can still interact with its value (e.g. mark and copy it).',
			'form-block'
		),
