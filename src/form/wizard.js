...
const fieldMatches = applyFilters( 'formBlock.wizard.fieldMatches', {
	checkbox: [
		_x(
			'checkbox',
			'potential form field name in lowercase',
			'form-block'
		),
...
export default function Wizard( props ) {
	const { clientId, isWizardOpen, setIsWizardOpen } = props;
	const [ fields, setFields ] = useState( '' );
	const [ includeConsentCheckbox, setIncludeConsentCheckbox ] =
		useState( false );
	...
	const onInsert = () => {
		...
		for ( const preparedField of preparedFields ) {
			...
			checkFieldTypeLoop: for ( const fieldType of Object.keys(
				fieldMatches
			) ) {
				for ( const potentialMatch of fieldMatches[ fieldType ] ) {
					if (
						! preparedField.toLowerCase().includes( potentialMatch )
					) {
						continue;
					}
